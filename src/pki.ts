...
export class PublicKey {
  ...
  static fromPEM(pem: Buffer): PublicKey {
    ...
    switch (msg.type) {
      ...
      default:
        throw new Error(
          "Could not convert public key from PEM, recommend PKCS#8 PEM"
        );
    }
  }
  ...
  private static _verifiers: { [index: string]: Verifier } = Object.create(
    null
  );
  ...
  verify(data: Buffer, signature: Buffer, hashAlgorithm: string): boolean {
    ...
    if (verifier != null) {
      const sum = createHash(hashAlgorithm)
        .update(data)
        .digest();
...
export class PrivateKey {
  ...
  static fromPEM(pem: Buffer): PrivateKey {
    ...
    switch (msg.type) {
      ...
      default:
        throw new Error(
          "Could not convert private key from PEM, recommend PKCS#8 PEM"
        );
    }
  }
  ...
  constructor(obj: ASN1) {
    ...
    if (EdDSAPrivateKeyOIDs.includes(this.oid)) {
      this._finalKey = this._keyRaw = ASN1.parseDER(
        this._keyRaw,
        Class.UNIVERSAL,
        Tag.OCTETSTRING
      ).bytes;
      if (this.oid === "1.3.101.112") {
        ...
        for (const val of obj.mustCompound()) {
          if (val.class === Class.CONTEXT_SPECIFIC && val.tag === 1) {
            ...
            this._finalKey = Buffer.concat([
              this._keyRaw,
              this._publicKeyRaw as Buffer
            ]);
          }
        }
      }
...
export class RSAPrivateKey extends PrivateKey {
  ...
  constructor(obj: ASN1) {
    ...
    this.publicExponent = ASN1.parseIntegerNum(
      captures.privateKeyPublicExponent.bytes
    );
    this.privateExponent = ASN1.parseIntegerStr(
      captures.privateKeyPrivateExponent.bytes
    );
    ...
    this.coefficient = ASN1.parseIntegerStr(
      captures.privateKeyCoefficient.bytes
    );
  }
...
PublicKey.addVerifier(getOID("Ed25519"), function(
  this: PublicKey,
  data: Buffer,
  signature: Buffer
): boolean {
...
PrivateKey.addSigner(getOID("Ed25519"), function(
  this: PrivateKey,
  data: Buffer
): Buffer {
  const key = this.keyRaw;
