...
export class Certificate {
  ...
  constructor(obj: ASN1) {
    ...
    this.raw = obj.DER;
    this.version =
      captures.certVersion == null
        ? 0
        : ASN1.parseIntegerNum(captures.certVersion.bytes) + 1;
    ...
    this.validFrom = ASN1.parseTime(
      captures.certValidityNotBefore.tag,
      captures.certValidityNotBefore.bytes
    );
    this.validTo = ASN1.parseTime(
      captures.certValidityNotAfter.tag,
      captures.certValidityNotAfter.bytes
    );
    ...
    if (captures.certIssuerUniqueId != null) {
      this.issuer.uniqueId = ASN1.parseBitString(
        captures.certIssuerUniqueId.bytes
      );
    }
    ...
    if (captures.certSubjectUniqueId != null) {
      this.subject.uniqueId = ASN1.parseBitString(
        captures.certSubjectUniqueId.bytes
      );
    }
  ...
  //   if (
  //     (this.version === 3 && !this.basicConstraintsValid) ||
  ...
  //   const res = this.publicKey.verify(
  //     child.tbsCertificate.DER,
  //     child.signature,
  //     agl
  //   );
  //   if (res === false) {
...
function decodeExtAuthorityKeyIdentifier(e: Extension) {
  const captures = ASN1.parseDERWithTemplate(
    e.value,
    authorityKeyIdentifierValidator
  );
  e.authorityKeyIdentifier = captures.authorityKeyIdentifier.bytes.toString(
    "hex"
  );
}
...
function fillMissingFields(attrs: Attribute[]) {
  for (const attr of attrs) {
    ...
    if (attr.shortName == null || attr.shortName === "") {
      attr.shortName =
        shortNames[attr.name] == null ? "" : shortNames[attr.name];
    }
